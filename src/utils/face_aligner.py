...
class FaceAligner(object):
    ...
    def align(self, image, bb, padding=45):
        ...
        face_aligned = face_aligned[self.padding: self.desiredFaceWidth - self.padding,
                       self.padding: self.desiredFaceWidth - self.padding]
        return face_aligned

    @staticmethod
    def img_process(img, input_shape):
        img = cv2.cvtColor(cv2.resize(img, input_shape), cv2.COLOR_BGR2RGB)
        return img
